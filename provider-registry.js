(function() {
  ...
  const BRAND = {
    name: 'Darkbrowser',
    shortName: 'Darkbrowser',
    description: 'Browser agent locked to your Dark LLM gateway'
  };

  const STORAGE_KEY = 'browserKingProviderState';
  const LEGACY_PROVIDER_KEY = 'providerConfig';

  // The only provider Darkbrowser is allowed to use. Every default and fallback below
  // resolves here, mirroring the darkcode CLI's hard lock to the Dark LLM gateway.
  const LOCKED_PROVIDER = 'darkllm';
    ...
    darkllm: {
      ...
      models: [
        createModel('qwen3-vl-8b-abliterated', 'Qwen3-VL-8B-abliterated', {
          supportsVision: true,
          description: '32K context · Vision (sees screenshots)',
        }),
  ...
  function buildDefaultState() {
    ...
    Object.values(PROVIDERS).forEach((provider) => {
      providers[provider.id] = {
        // Only the locked Dark LLM provider is enabled by default; the others stay in the
        // registry (so the code path exists) but are hidden and never active.
        enabled: provider.id === LOCKED_PROVIDER,
        apiKey: '',
  ...
  function normalizeState(input) {
    ...
    if (locked) {
      ...
      if (!laneIds.includes(locked.model)) {
        locked.model = laneIds[0] || 'qwen3-vl-8b-abliterated';
      }
    }
  ...
  // Legacy z.ai state migration - unused now that Darkbrowser locks to Dark LLM. Kept as a no-op
  // so old callers do not break; it must never flip the active provider away from the lock.
  function migrateZaiProviders() {
    return;
  }

  function migrateZaiProvidersLegacy(state) {
  ...
  function getProviderDefinition(providerId) {
    return PROVIDERS[providerId] || PROVIDERS[LOCKED_PROVIDER];
  }
  ...
  function getActiveProviderState(state) {
    return state.providers[state.activeProvider] || state.providers[LOCKED_PROVIDER];
  }
  ...
  async function syncStateToChrome(state) {
    ...
    await chrome.storage.local.set({
      ...
      // The stock extension needs *some* value here to not show Claude's own login. The real
      // credential that Darkbrowser routes with is activeState.apiKey (the Dark LLM key); this
      // placeholder is never sent upstream. See the sign-in gate in api-adapter.js.
      anthropicApiKey: activeState.apiKey || 'darkbrowser-signed-out',
